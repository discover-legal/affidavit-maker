...
describe('document generation entitlement input', () => {
  ...
  it('accepts a missing document ID at the schema level (runtime-required only when payments are enabled)', () => {
    // The PAYMENTS_ENABLED kill-switch allows free ad-hoc generation without
    // a saved document; the route enforces documentId at runtime when
    // payments are on. See e2e/drive.mjs step 7 for the contract.
    expect(generateSchema.safeParse({ affidavitData }).success).toBe(true);
  });
