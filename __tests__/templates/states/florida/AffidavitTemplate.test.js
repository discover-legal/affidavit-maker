// __tests__/templates/states/florida/AffidavitTemplate.test.js
const FloridaAffidavitTemplate = require('../../../../templates/states/florida/AffidavitTemplate');
const BaseAffidavitTemplate = require('../../../../templates/core/BaseAffidavitTemplate');

describe('FloridaAffidavitTemplate', () => {
  let template;

  beforeEach(() => {
    template = new FloridaAffidavitTemplate();
  });

  describe('constructor', () => {
    it('should extend BaseAffidavitTemplate', () => {
      expect(template).toBeInstanceOf(BaseAffidavitTemplate);
    });

    it('should set state code to FL', () => {
      expect(template.state).toBe('FL');
    });

    it('should set state name to Florida', () => {
      expect(template.stateName).toBe('Florida');
    });

    it('should load metadata', () => {
      expect(template.metadata).toBeDefined();
      expect(template.metadata.stateCode).toBe('FL');
    });

    it('should not include perjury statement', () => {
      expect(template.sections.perjuryStatement).toBe(false);
    });

    it('should require county', () => {
      expect(template.requiredFields).toContain('county');
    });
  });

  describe('generateHeader', () => {
    it('should generate correct Florida header', () => {
      const header = template.generateHeader();
      expect(header).toBe('STATE OF FLORIDA');
    });
  });

  describe('generateVenue', () => {
    it('should generate sentence case venue', () => {
      const venue = template.generateVenue('Miami-Dade');
      expect(venue).toBe('State of Florida\nCounty of Miami-Dade');
    });

    it('should handle county names without altering case', () => {
      const venue = template.generateVenue('Palm Beach');
      expect(venue).toBe('State of Florida\nCounty of Palm Beach');
    });

    it('should use placeholder when county not provided', () => {
      const venue = template.generateVenue('');
      expect(venue).toContain('[COUNTY NAME]');
    });
  });

  describe('generateCaseCaption', () => {
    it('should use CASE NO. terminology', () => {
      const data = { caseNumber: '2024-CA-12345' };
      const caption = template.generateCaseCaption(data);

      expect(caption.formatted).toContain('CASE NO.');
      expect(caption.formatted).toContain('2024-CA-12345');
    });

    it('should include court name', () => {
      const data = {
        court: 'Circuit Court, Miami-Dade County',
        caseNumber: '2024-12345'
      };
      const caption = template.generateCaseCaption(data);

      expect(caption.formatted).toContain('CIRCUIT COURT, MIAMI-DADE COUNTY');
    });

    it('should use vs. format for parties', () => {
      const data = {
        plaintiff: 'John Doe',
        defendant: 'Jane Smith',
        caseNumber: '2024-12345'
      };
      const caption = template.generateCaseCaption(data);

      expect(caption.formatted).toContain('JOHN DOE');
      expect(caption.formatted).toContain('Plaintiff');
      expect(caption.formatted).toContain('vs.');
      expect(caption.formatted).toContain('JANE SMITH');
      expect(caption.formatted).toContain('Defendant');
    });

    it('should use placeholders when data missing', () => {
      const data = {};
      const caption = template.generateCaseCaption(data);

      expect(caption.formatted).toContain('[COURT NAME]');
      expect(caption.formatted).toContain('[CASE NUMBER]');
    });
  });

  describe('generateNotaryBlock', () => {
    it('should generate Florida statutory notary block', () => {
      const notaryBlock = template.generateNotaryBlock({});

      expect(notaryBlock).toContain('State of Florida');
      expect(notaryBlock).toContain('Sworn to (or affirmed) and subscribed');
      expect(notaryBlock).toContain('Notary Public - State of Florida');
    });

    it('should include physical/online notarization checkboxes', () => {
      const notaryBlock = template.generateNotaryBlock({});
      expect(notaryBlock).toContain('physical presence');
      expect(notaryBlock).toContain('online notarization');
      expect(notaryBlock).toMatch(/☐/); // Contains checkbox character
    });

    it('should include identification verification', () => {
      const notaryBlock = template.generateNotaryBlock({});
      expect(notaryBlock).toContain('Personally Known');
      expect(notaryBlock).toContain('Produced Identification');
      expect(notaryBlock).toContain('Type of Identification');
    });
  });

  describe('generatePerjuryStatement', () => {
    it('should return null (no perjury statement for Florida)', () => {
      const statement = template.generatePerjuryStatement();
      expect(statement).toBeNull();
    });
  });

  describe('performStateSpecificValidation', () => {
    it('should require county', () => {
      const data = {
        affiantName: 'John Doe',
        state: 'FL'
      };

      const result = template.performStateSpecificValidation(data);
      expect(result.errors).toContain('County is required for Florida affidavits');
    });

    it('should pass with county provided', () => {
      const data = {
        affiantName: 'John Doe',
        state: 'FL',
        county: 'Miami-Dade'
      };

      const result = template.performStateSpecificValidation(data);
      expect(result.errors).toHaveLength(0);
    });

    it('should warn about perjury statement in facts', () => {
      const data = {
        affiantName: 'John Doe',
        state: 'FL',
        county: 'Miami-Dade',
        facts: ['I declare under penalty of perjury that...']
      };

      const result = template.performStateSpecificValidation(data);
      expect(result.warnings.length).toBeGreaterThan(0);
    });
  });

  describe('getExhibitRules', () => {
    it('should use letter labels', () => {
      const rules = template.getExhibitRules();
      expect(rules.labelStyle).toBe('letters');
    });

    it('should require cover pages', () => {
      const rules = template.getExhibitRules();
      expect(rules.requireCoverPage).toBe(true);
    });

    it('should include instructions', () => {
      const rules = template.getExhibitRules();
      expect(rules.instructions).toBeDefined();
      expect(rules.instructions).toContain('exhibit');
    });
  });

  describe('generateDocument', () => {
    it('should generate complete document', () => {
      const data = {
        affiantName: 'John Doe',
        state: 'FL',
        county: 'Miami-Dade',
        facts: ['Fact 1', 'Fact 2']
      };

      const doc = template.generateDocument(data);

      expect(doc.id).toBeDefined();
      expect(doc.state).toBe('FL');
      expect(doc.sections).toBeDefined();
      expect(doc.fullText).toBeDefined();
      expect(doc.htmlContent).toBeDefined();
    });

    it('should not include perjury statement section', () => {
      const data = {
        affiantName: 'John Doe',
        state: 'FL',
        county: 'Miami-Dade',
        facts: ['Fact 1']
      };

      const doc = template.generateDocument(data);
      expect(doc.sections.perjuryStatement).toBeNull();
    });

    it('should include Florida header', () => {
      const data = {
        affiantName: 'John Doe',
        state: 'FL',
        county: 'Miami-Dade',
        facts: ['Fact 1']
      };

      const doc = template.generateDocument(data);
      expect(doc.sections.header).toBe('STATE OF FLORIDA');
    });

    it('should include proper venue', () => {
      const data = {
        affiantName: 'John Doe',
        state: 'FL',
        county: 'Miami-Dade',
        facts: ['Fact 1']
      };

      const doc = template.generateDocument(data);
      expect(doc.sections.venue).toContain('State of Florida');
      expect(doc.sections.venue).toContain('County of Miami-Dade');
    });
  });

  describe('legal compliance', () => {
    it('should have legal citations in metadata', () => {
      expect(template.metadata.legalCitations).toBeDefined();
      expect(template.metadata.legalCitations.length).toBeGreaterThan(0);
    });

    it('should reference Florida statutes', () => {
      const citations = template.metadata.legalCitations;
      const hasFLStat = citations.some(c => c.code.includes('Fla. Stat.'));
      expect(hasFLStat).toBe(true);
    });

    it('should be marked as legally compliant', () => {
      expect(template.metadata.legallyCompliant).toBe(true);
    });

    it('should have version 2.0', () => {
      expect(template.metadata.version).toBe('2.0');
    });
  });
});
