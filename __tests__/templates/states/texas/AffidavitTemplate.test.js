...
const TexasAffidavitTemplate = require('../../../../templates/states/texas/AffidavitTemplate');
const BaseAffidavitTemplate = require('../../../../templates/core/BaseAffidavitTemplate');

describe('TexasAffidavitTemplate', () => {
  let template;

  beforeEach(() => {
    template = new TexasAffidavitTemplate();
  });
  ...
  describe('constructor', () => {
    ...
    it('should set state code to TX', () => {
      expect(template.state).toBe('TX');
    });

    it('should set state name to Texas', () => {
      expect(template.stateName).toBe('Texas');
    });

    it('should load metadata', () => {
      expect(template.metadata).toBeDefined();
      expect(template.metadata.stateCode).toBe('TX');
    });
  ...
  describe('generateHeader', () => {
    it('should generate correct Texas header', () => {
      const header = template.generateHeader();
      expect(header).toBe('THE STATE OF TEXAS');
    });
  });

  describe('generateVenue', () => {
    it('should generate uppercase venue', () => {
      const venue = template.generateVenue('Travis');
  ...
  describe('generateCaseCaption', () => {
    it('should use CAUSE NO. terminology', () => {
      ...
      expect(caption.formatted).toContain('CAUSE NO.');
    ...
    it('should include court name', () => {
      ...
      expect(caption.formatted).toContain('DISTRICT COURT OF TRAVIS COUNTY');
    });
  ...
  describe('generateNotaryBlock', () => {
    it('should generate Texas statutory notary block', () => {
    ...
    it('should include notary printed name line', () => {
      const notaryBlock = template.generateNotaryBlock({});
      expect(notaryBlock).toContain('Notary\'s printed name');
    });
  });

  describe('generatePerjuryStatement', () => {
    it('should return null (no perjury statement for Texas)', () => {
  ...
  describe('generateDocument', () => {
    ...
    it('should include Texas header', () => {
      const data = {
        affiantName: 'John Doe',
        state: 'TX',
        county: 'Travis',
    ...
    it('should include proper venue', () => {
      ...
      const doc = template.generateDocument(data);
      expect(doc.sections.venue).toBe('COUNTY OF TRAVIS');
    });
  });
  ...
  describe('legal compliance', () => {
    ...
    it('should reference Texas Government Code', () => {
      const citations = template.metadata.legalCitations;
      const hasTexCode = citations.some(c => c.code.includes('Tex.'));
      expect(hasTexCode).toBe(true);
    });
