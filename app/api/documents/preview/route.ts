    ...
    try {
      if (isDivorce) {
        ...
        if (templateManager.hasDocumentType(state, effectiveDocType)) {
          ...
          // Supporting packet documents are generated by the shared divorce
          // generator rather than registered state template classes. They are
          // still first-class preview tabs and must never fall back to a fake
          // "Original Petition" placeholder.
          const divorceDocumentGenerator = require('@/services/documents/DivorceDocumentGenerator') as DivorceDocumentGenerator;
          document = divorceDocumentGenerator.generate(state, effectiveDocType, divorceData);
        }
      } else {
