// Terms of Service Content
// Last Updated: November 12, 2025

export const TOS_VERSION = "1.1.0";
export const TOS_LAST_UPDATED = "November 12, 2025";

export const TERMS_OF_SERVICE = `
# Terms of Service

**Last Updated: ${TOS_LAST_UPDATED}**

Welcome to Affidavit Maker! We're glad you're here. We've written these terms in plain English to make them easy to understand. Please read them carefully.

## The Basics

By using Affidavit Maker, you agree to these terms. If you don't agree, please don't use our service.

## What We Do

Affidavit Maker helps you create legal affidavits and documents using AI technology. We provide tools to draft, edit, and generate professional legal documents. However, **we are not a law firm and do not provide legal advice**. For legal advice, please consult a licensed attorney.

## Your Account

- You must provide accurate information when creating your account
- You're responsible for keeping your account credentials secure
- You must be at least 18 years old to use our service
- One account per person - sharing accounts isn't allowed
- You're responsible for all activity that happens under your account

## Your Data & Privacy - What Makes Us Different

**Here's what we promise about your data:**

### 🔒 We Will NEVER Sell Your Data to Advertisers

Your personal information, documents, and data are yours. Period. We will never sell, rent, or share your information with advertisers or data brokers. This is a core value of our company and will never change.

### ✅ What We DO With Your Data:

- Store it securely to provide you with our service
- Use it to improve our AI and service features
- Keep it backed up so you never lose your work
- Process payments through our trusted payment processor (Stripe)
- Share only what's legally required (like valid court orders)

### 🚫 What We DON'T Do With Your Data:

- Sell it to anyone, ever
- Share it with advertisers or marketing companies
- Use it to train third-party AI models without your permission
- Give third parties access without your explicit consent
- Read your documents unless you ask us for support

### 🎓 Academic Research & Data for Good

We believe in advancing knowledge and improving legal access. With your consent through acceptance of these terms, we may use **de-identified, aggregated data** for academic research purposes:

**What this means:**
- We may share anonymized, non-identifiable data with academic researchers
- All personal information, names, and identifying details are removed
- Research focuses on improving legal document accessibility and AI technology
- Results may be published in academic papers or conferences
- This helps improve legal services for everyone

**Your protection:**
- All data is de-identified using industry-standard anonymization techniques
- No individual documents or cases can be traced back to you
- We comply with all research ethics standards (IRB requirements)
- You can opt out of research data sharing at any time in your account settings
- Opting out does not affect your use of the service

**Examples of research use:**
- Studying patterns in legal document creation to improve AI assistance
- Analyzing how people use legal services to increase accessibility
- Publishing anonymized statistics about legal document trends
- Improving natural language processing for legal documents

This research clause supports our mission to make legal services more accessible while maintaining your privacy and never selling your data.

## Payment & Billing

- Our service costs $79 per completed document
- All payments are processed securely through Stripe
- Payments are non-refundable once a document is completed and downloaded
- We reserve the right to change pricing with 30 days notice
- Failed payments may result in service suspension

## What You Can and Can't Do

### ✅ You CAN:

- Create documents for personal or business use
- Download and use documents you've paid for
- Share your completed documents as needed
- Export your data at any time

### 🚫 You CANNOT:

- Use our service for illegal activities
- Upload malicious code or viruses
- Attempt to hack, reverse engineer, or compromise our system
- Resell or redistribute our service
- Create documents containing false information
- Impersonate others or misrepresent yourself
- Use our service to harass, abuse, or harm others
- Scrape or data mine our service

## Intellectual Property

- You own the content you create and the documents you generate
- We own the platform, code, and technology that powers Affidavit Maker
- You grant us a limited license to process your content to provide our service
- Our logos, trademarks, and branding belong to us

## AI-Generated Content Disclaimer

Our service uses AI technology to help create documents. While we strive for accuracy:

- **AI can make mistakes** - Always review your documents carefully
- **AI is not a lawyer** - This is not legal advice
- **You're responsible** for the accuracy of your final documents
- **Double-check everything** before using documents in legal proceedings
- **Consult an attorney** if you're unsure about anything

## Service Availability

- We strive for 99.9% uptime but can't guarantee it
- We may perform maintenance that temporarily interrupts service
- We're not liable for service interruptions or downtime
- We'll notify you of planned maintenance when possible

## Limitation of Liability

**To the maximum extent permitted by law:**

- Our service is provided "as is" without warranties
- We're not liable for any indirect, incidental, or consequential damages
- Our total liability is limited to the amount you paid us in the last 12 months
- We're not responsible for the legal validity of documents you create
- We're not liable for outcomes of legal proceedings using our documents

## Termination

### We May Suspend or Terminate Your Account If You:

- Violate these terms
- Use the service fraudulently
- Don't pay for services
- Engage in abusive behavior
- Do anything that harms our service or other users

### You Can Cancel Anytime:

- You can stop using our service whenever you want
- You can delete your account in your account settings
- You'll still have access to documents you've paid for
- Cancellation doesn't entitle you to a refund

## Changes to These Terms

**We can update these terms at any time.** Here's how it works:

- We'll notify you by email when we make significant changes
- We'll also update the "Last Updated" date at the top
- Continued use after changes means you accept the new terms
- If you don't agree with changes, you should stop using the service
- We recommend checking these terms periodically

For material changes that affect your rights, we'll provide at least 30 days notice.

## Data Security

We take security seriously:

- All data is encrypted in transit (HTTPS/TLS)
- All data is encrypted at rest
- We use industry-standard security practices
- We perform regular security audits
- We limit employee access to your data
- We'll notify you of any data breaches as required by law

However, no system is 100% secure. You use our service at your own risk.

## Your Data Rights

You have the right to:

- Access your data at any time
- Export your data in common formats
- Delete your account and data
- Correct inaccurate information
- Opt out of marketing communications

To exercise these rights, contact us at support@affidavitmaker.com (or through your account settings).

## Children's Privacy

Our service is not intended for anyone under 18. We don't knowingly collect information from children. If we discover we have collected data from a child, we'll delete it immediately.

## Governing Law

These terms are governed by the laws of the United States and the state where our company is registered, without regard to conflict of law provisions. Any disputes will be resolved in the courts located in that jurisdiction.

## Dispute Resolution

- We prefer to resolve disputes informally - contact us first
- If we can't resolve it, we'll try mediation
- You agree to resolve disputes individually, not as part of a class action
- You have the right to opt out of arbitration within 30 days of accepting these terms

## Entire Agreement

These terms, along with our Privacy Policy, constitute the entire agreement between you and Affidavit Maker. They supersede any prior agreements or understandings.

## Severability

If any part of these terms is found to be unenforceable, the rest remains in full effect.

## No Waiver

Our failure to enforce any right or provision doesn't constitute a waiver of that right.

## Contact Us

Questions about these terms? We're here to help!

**Email:** support@affidavitmaker.com

---

## Summary (TL;DR)

- Be honest and don't do anything illegal or harmful
- **We will NEVER sell your data to advertisers**
- We may use de-identified data for academic research (you can opt out)
- We're a tool, not a law firm - review your documents carefully
- $79 per document, non-refundable after download
- We can update these terms with notice
- You own your documents, we own the platform
- You can cancel anytime
- Be nice to others and follow the rules

**By clicking "I Accept" below, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service.**
`;
