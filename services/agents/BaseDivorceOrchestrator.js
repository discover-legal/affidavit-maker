    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          case_number: {
            type: 'string',
            description: "MACHINE-READ court file / case number when the user names one (e.g. Ontario 'FS-26-00001234', TX 'D-1-FM-25-000123', CA 'FLDS-25-00012345', NY 'IDV-000042-2025'). Populate with the exact identifier as the user typed it, trimmed and case-preserved (Ontario file numbers are uppercase; leave punctuation the user used). Only populate when the user names a court file/case/docket/cause number for THIS case — never invent one, never carry an ex-spouse's prior case number, never populate from a fact that only references a different matter. When the user has not named one, OMIT this field entirely (do NOT emit placeholders like 'unknown', 'TBD', 'pending', '[COURT FILE NUMBER]'). This value renders as the Court File No. in every jurisdiction's caption (Ontario Form 10 + Divorce Order, TX Cause No., CA/NY Case No.). Recording the number only as a fact is a bug: the caption reads the STRUCTURED field.",
          },
          service_date: {
            type: 'string',
