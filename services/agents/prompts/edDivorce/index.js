...
const RESIDENCY = `Residency for Edo State. MCA s.2: domicile or 3 years. Filing allowed in any state's High Court (MCA s.2(3)); Edo High Court is the usual venue if either party resides in the state.\nCOLLECT: domicile, residence in Edo, judicial division (e.g., Benin, Auchi, Ekpoma).\n${SHARED_RULES}`;
...
const REVIEW = `Final review. Filing fees ~NGN 10,000-30,000. Decree Nisi -> 3 months -> Decree Absolute (automatic, s.58; s.57 declaration where children under 16). A4.\n${SHARED_RULES}`;
const PHASES = {
