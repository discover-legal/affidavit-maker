...
class BCDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  ...
  generateChildCustodySection(divorceData) {
    ...
    if (custodyType === 'joint') {
      ...
      items.push({
        content: `IT IS ORDERED that ${divorceData.primaryCustodian || divorceData.petitionerName || 'Claimant'} shall have sole decision-making responsibility for the child(ren) pursuant to the Divorce Act, RSC 1985, c. 3, s.16.1.`,
        type: 'order'
      });
      items.push({
        content: `IT IS ORDERED that ${divorceData.respondentName || 'Respondent'} shall have parenting time with the child(ren) as set out in a parenting schedule attached to this Order.`,
        type: 'order'
      });
    }
