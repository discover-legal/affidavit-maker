...
class EnuguAffidavitTemplate extends BaseAffidavitTemplate {
  ...
  generateHeader() { return 'IN THE HIGH COURT OF ENUGU STATE'; }
  generateVenue(county) { return `${(county || '[JUDICIAL DIVISION]').toUpperCase()} JUDICIAL DIVISION`; }
  generateCaseCaption(d) { const court = d.court || d.courtName || 'HIGH COURT OF ENUGU STATE'; const p = d.plaintiff || d.petitionerName || '[PETITIONER NAME]'; const r = d.defendant || d.respondentName || '[RESPONDENT NAME]'; return { courtName: court, caseNumber: d.caseNumber, plaintiff: p, defendant: r, formatted: `IN THE ${court.toUpperCase()}\n${(d.county || d.city || '[JUDICIAL DIVISION]').toUpperCase()} JUDICIAL DIVISION\n\nSuit No. ${d.caseNumber || '[SUIT NUMBER]'}\n\n${p.toUpperCase()}\nPetitioner\n\n— AND —\n\n${r.toUpperCase()}\nRespondent` }; }
  performStateSpecificValidation(d) { const errors = [], warnings = []; if (!d.county && !d.city) errors.push('Judicial division required for Enugu State affidavits.'); return { errors, warnings }; }
...
module.exports = EnuguAffidavitTemplate;
