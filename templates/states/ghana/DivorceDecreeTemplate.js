...
class GhanaDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  ...
  generateChildCustodySection(divorceData) {
    ...
    if (custodyType === 'joint') {
      ...
      items.push({
        content: `IT IS ORDERED that ${divorceData.primaryCustodian || divorceData.petitionerName || 'Petitioner'} shall have sole custody of the child(ren).`,
        type: 'order'
      });
      items.push({
        content: `IT IS ORDERED that ${divorceData.respondentName || 'Respondent'} shall have reasonable access to the child(ren) as agreed by the parties or as determined by this Court.`,
        type: 'order'
      });
    }
