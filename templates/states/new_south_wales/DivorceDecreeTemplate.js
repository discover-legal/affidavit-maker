...
class NewSouthWalesDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  ...
  generateChildCustodySection(divorceData) {
    ...
    if (custody.kind === 'joint') {
      ...
      items.push({
        content: `IT IS ORDERED that the child(ren) shall live with ${resolvePrimaryResidenceName(divorceData) || divorceData.petitionerName || 'Applicant'} and spend time with ${divorceData.respondentName || 'Respondent'} as agreed or as set out in a parenting plan.`,
        type: 'order'
      });
      ...
      items.push({
        content: `IT IS ORDERED that ${custodianName} shall have sole parental responsibility for the child(ren) pursuant to the Family Law Act 1975 (Cth).`,
        type: 'order'
      });
      items.push({
        content: `IT IS ORDERED that the child(ren) shall spend time with ${otherParentName} as agreed by the parties or as determined by the Court.`,
        type: 'order'
      });
    } else {
