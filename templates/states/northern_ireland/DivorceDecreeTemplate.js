...
class NorthernIrelandDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  ...
  generateChildCustodySection(divorceData) {
    ...
    if (custody.kind === 'joint') {
      ...
      items.push({
        content: `IT IS ORDERED that a residence order be made in respect of the child(ren), such that the child(ren) shall reside with ${custodianName}.`,
        type: 'order'
      });
      items.push({
        content: `IT IS ORDERED that a contact order be made, such that ${otherParentName} shall have contact with the child(ren) as agreed between the parties or as set out in a schedule attached to this order.`,
        type: 'order'
      });
    } else {
