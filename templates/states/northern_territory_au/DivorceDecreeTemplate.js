// templates/states/northern_territory_au/DivorceDecreeTemplate.js
'use strict';
const BaseDivorceDecreeTemplate = require('../../core/BaseDivorceDecreeTemplate');
const { resolveCustodyArrangement, resolvePrimaryResidenceName } = require('../../core/parenting');

class NorthernTerritoryAUDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  ...
  constructor() {
    super();
    this.state = 'NT_AU'; this.stateName = 'Northern Territory'; this.countryCode = 'AU'; this.terminology = { ...this.terminology, jurisdictionLabel: null, districtLabel: null, districtStyle: 'plain', jurisdictionTerm: 'Territory', districtTerm: 'Registry', districtPlaceholder: '[REGISTRY]', filerLabel: 'Applicant', responderLabel: 'Respondent', selfRepresentedLabel: 'Self-Represented' }; this.documentTitle = 'DIVORCE ORDER';
  ...
  getDefaultCourt(county) { return `FEDERAL CIRCUIT AND FAMILY COURT OF AUSTRALIA (DIVISION 2) — ${(county || 'DARWIN').toUpperCase()} REGISTRY`; }
  generateHeader() { return 'NORTHERN TERRITORY'; }
  generateVenue(county) { return `${(county || 'DARWIN').toUpperCase()} REGISTRY`; }
  ...
  getCertificateNote() { return 'A sealed copy may be obtained from the Darwin registry after the order takes effect.'; }
}
module.exports = NorthernTerritoryAUDivorceDecreeTemplate;
