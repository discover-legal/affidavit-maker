// templates/states/rivers/DivorceDecreeTemplate.js
// Rivers State divorce decree template
...
class RiversDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  constructor() {
    super();
    this.state = 'RV';
  ...
  generateChildCustodySection(divorceData) {
    ...
    if (custodyType === 'joint') {
      items.push({ content: `IT IS ORDERED that ${divorceData.petitionerName || 'the Petitioner'} and ${divorceData.respondentName || 'the Respondent'} shall have joint custody of the child(ren) pursuant to section 71 of the Matrimonial Causes Act.`, type: 'order' });
    } else {
      items.push({ content: `IT IS ORDERED that ${divorceData.primaryCustodian || divorceData.petitionerName || 'the Petitioner'} shall have sole custody of the child(ren) pursuant to section 71 of the Matrimonial Causes Act.`, type: 'order' });
      items.push({ content: `IT IS ORDERED that ${divorceData.respondentName || 'the Respondent'} shall have reasonable access to the child(ren).`, type: 'order' });
    }
    items.push({ content: 'IT IS ORDERED that neither party shall do anything to alienate the child(ren)\'s affection for the other party.', type: 'order' });
  ...
  generateChildSupportSection(divorceData) {
    ...
    if (divorceData.childSupportAmount) {
      items.push({ content: `IT IS ORDERED pursuant to section 70 of the Matrimonial Causes Act that ${divorceData.childSupportObligor || divorceData.respondentName || 'the Respondent'} shall pay NGN ${divorceData.childSupportAmount} per month for the maintenance of the child(ren).`, type: 'order' });
    } else {
      items.push({ content: 'IT IS ORDERED pursuant to section 70 of the Matrimonial Causes Act that the Respondent shall pay such sum as the Court deems reasonable for the maintenance of the child(ren).', type: 'order' });
    }
  ...
  generateSpousalSupportSection(divorceData) {
    ...
    if (divorceData.spousalSupportWaived) {
      items.push({ content: 'The Court notes that neither party has applied for maintenance under section 70 of the Matrimonial Causes Act.', type: 'order' });
  ...
  generateFinalOrdersSection(divorceData) {
    ...
    items.push({ content: 'Either party may apply to this Court for the Decree Nisi to be made Absolute after the expiration of the said three months. A certified copy of the Decree Absolute may be obtained from the Court Registry.', type: 'order' });
    return { title: 'FINAL ORDERS', items, type: 'final_orders' };
  }
}

module.exports = RiversDivorceDecreeTemplate;
