...
class SouthAfricaDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  ...
  generateChildCustodySection(divorceData) {
    ...
    if (custody.kind === 'joint') {
      ...
      items.push({
        content: `IT IS ORDERED that the child(ren) shall be in the primary care of ${resolvePrimaryResidenceName(divorceData) || divorceData.petitionerName || 'Plaintiff'}, with reasonable contact to ${divorceData.respondentName || 'Defendant'} as agreed between the parties or as set out in the Parenting Plan.`,
        type: 'order'
      });
      ...
      items.push({
        content: `IT IS ORDERED that ${custodianName} shall have sole care of the minor child(ren) in terms of the Children's Act 38 of 2005.`,
        type: 'order'
      });
      items.push({
        content: `IT IS ORDERED that ${otherParentName} shall have reasonable contact with the child(ren) as agreed between the parties or as determined by this Court.`,
        type: 'order'
      });
    } else {
