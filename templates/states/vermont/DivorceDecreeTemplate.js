...
class VermontDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  ...
  generateChildCustodySection(divorceData) {
    ...
    if (custodyType === 'joint') {
      items.push({
        content: `IT IS ORDERED that the parties shall share Legal Responsibility (joint decision-making authority) for the minor child(ren). The primary Physical Responsibility (where the child resides) shall be with ${divorceData.primaryCustodian || divorceData.petitionerName || 'Plaintiff'}.`,
        type: 'order'
      });
    } else {
      items.push({
        content: `IT IS ORDERED that ${divorceData.primaryCustodian || divorceData.petitionerName || 'Plaintiff'} shall have sole Legal Responsibility and primary Physical Responsibility for the minor child(ren).`,
        type: 'order'
      });
    }
