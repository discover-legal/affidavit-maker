// templates/states/victoria/DivorceDecreeTemplate.js
 ...
 * Victoria Divorce Order Template
 *
 * @class VictoriaDivorceDecreeTemplate
 * @extends BaseDivorceDecreeTemplate
 */
class VictoriaDivorceDecreeTemplate extends BaseDivorceDecreeTemplate {
  constructor() {
    super();
  ...
  generateChildCustodySection(divorceData) {
    ...
    const items = [];
    items.push({ content: 'The Court finds that the following parenting orders are in the best interests of the child(ren) (Family Law Act 1975 (Cth), s.60CA):', type: 'finding' });
  ...
  getVisitationLanguage(divorceData) {
    return 'IT IS ORDERED that each party shall facilitate and encourage the child(ren)\'s relationship with the other party (Family Law Act 1975 (Cth), s.60CC).';
  }
  ...
  generateChildSupportSection(divorceData) {
    ...
    if (divorceData.childSupportAmount) {
      items.push({ content: `IT IS ORDERED that ${divorceData.childSupportObligor || divorceData.respondentName || 'Respondent'} shall pay child support of $${divorceData.childSupportAmount} per month, or as assessed by Services Australia (Child Support).`, type: 'order' });
    } else {
  ...
  generateFinalOrdersSection(divorceData) {
    ...
    items.push({ content: 'IT IS ORDERED that all claims for relief not expressly granted herein are dismissed.', type: 'order' });
    items.push({ content: 'IT IS ORDERED that each party shall execute such further documents as may be necessary to give effect to this Order.', type: 'order' });
  ...
  getEffectiveDateText() {
    return 'This Divorce Order takes effect one month and one day after the date on which it is made (Family Law Act 1975 (Cth), s.55). The parties may not remarry until the order takes effect.';
  }
...
module.exports = VictoriaDivorceDecreeTemplate;
